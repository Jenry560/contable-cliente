...
export default function TranProveedor({env, base, calcular,value,liquidar}){
...
function filtrar(){
    ...
    if(filtValue){
        const ValorAfiltrar = codigo.filter((ap)=> Object.values(ap).some(valor => String(valor).includes(filtValue)))
   ...
   const formatoNumero = new Intl.NumberFormat('en-US');
    ...
    return(
        ...
        <table>
                ...
                <td className="moditr">{formatoNumero.format(bas.Monto)}</td>
                <Trans id={bas.seq} calcular={calcular}>{bas.Saldo}</Trans>
                
              </tr>)) 
                : 
                codigo.map((bas,index) =>( <tr className="moditr" key={index}>
                    <td className="moditr">{`${bas.Asiento}-000${bas.seq}`}</td>
                    <td className="moditr">{bas.Fecha}</td>
                    <td className="moditr">{bas.Descripcion}</td>
                    <td className="moditr">{formatoNumero.format(bas.Monto)}</td>
            ...
            {codigo.length === 0 &&  <p className="trans">No hay transaccion en el proveedor</p>}
