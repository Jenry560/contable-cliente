...
export default function Cliente({ RegistradoPor }) {
  ...
  const fechData = async () => {
    ...
    const filtro = ClienteData.data.filter(
      (fil) => fil.Registro == RegistradoPor
    );
    if (filtro.length > 0) {
      setDatos(filtro);
    }
  ...
  const fechDataTrans = async () => {
    ...
    const filtro = TransData.data.filter(
      (fil) => fil.Registro == RegistradoPor
    );
    if (filtro.length > 0) {
      setBase(filtro);
      ...
      if (number.length === 2) {
        ...
        if (!hay_negativo && hay_positivo) {
          ...
          for (var id_seq of number) {
            if (id_seq.numbe > 0) {
              valor_positivo = id_seq.Id;
            } else if (id_seq.numbe < 0) {
              valor_Negativo = id_seq.Id;
            }
          }
          const dataLiqui = {
          ...
          const respuesta = await axios.post(
            "https://contableserver.azurewebsites.net/liquidar",
            dataLiqui
          );

          if (respuesta) {
            if (value >= 0) {
              const buscarNegativo = base.find(
                (dat) => dat.seq === valor_Negativo
              );
              ...
              const RegistroCorrecto = await axios.post(
              ...
              const nuevoMonto =
                parseInt(buscarNegativo.Monto) * -1 + parseInt(value);
          ...
          alert(
            "Solo puede selecionar un debito y credito para poder liquidar la tra"
          );
        }
      } else if (number.length > 2) {
        alert(
          "Has excedido el numero de dos trasacciones para poder hacer la funcion de liquidar"
        );
      } else if (number.length < 2) {
        alert(
          "Debes selecionar dos trasacciones para poder hacer la funcion de liquidar"
        );
      }
    } catch (error) {
      console.log(error);
  ...
  function filtrar() {
    ...
    if (filtValue) {
      const ValorAfiltrar = datos.filter((ap) =>
        ...
        <>
          <div>
            ...
            <div className="newmove">
              <BiPrinter
                className={visibility ? "printClient" : "visibility"}
                onClick={imprimir}
              />
              <RiFileExcel2Fill
                className={visibility ? "printClient" : "visibility"}
            ...
            <div className="container_cliente" id="miTabla">
              ...
              {datos.length === 0 && (
                <p className="trans">No hay clientes registrado</p>
              )}
            </div>
          </div>
          <br />
